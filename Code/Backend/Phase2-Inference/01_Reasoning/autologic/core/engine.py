...
class AutoLogicEngine:
    ...
    async def select_modules(self, task: str, root_llm: Optional[BaseLLM] = None) -> List[ReasoningModule]:
        ...
        modules_json = json.dumps(
            [m.to_dict() for m in self.reasoning_modules],
            indent=2,
            ensure_ascii=False
        )
    ...
    async def adapt_modules(
        ...
        try:
            ...
            for item in adapted_data.get('adapted_modules', []):
                base_module = next(
                    (m for m in selected if m.id == item['id']),
                    None
                )
                if base_module:
                    adapted_modules.append(AdaptedModule(
                        base=base_module,
