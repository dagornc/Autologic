...
class OpenRouterLLM(BaseLLM):
    ...
    async def _get_fallback_model(self) -> Optional[str]:
        ...
        available = [
            m for m in OpenRouterLLM._free_models_cache 
        ...
        **kwargs: Any
        ...
        if not self._api_key:
            raise ValueError(
                ...
                "Définir OPENAI_API_KEY ou passer api_key."
            )
...
class OllamaLLM(BaseLLM):
    ...
    @classmethod
    async def list_models(cls, host: Optional[str] = None, api_key: Optional[str] = None, raise_error: bool = False, **kwargs: Any) -> List[str]:
        """
...
class HuggingFaceLLM(BaseLLM):
    ...
    async def call(self, prompt: str, **kwargs: Any) -> str:
        """Exécute un appel à l'API HuggingFace Inference."""
        try:
